...
Развёрнутая спека (токены, шкала кеглей, сетки) лежит в `docs/DESIGN.md` —
каталог `docs/` исключён из репозитория локально, так что файл есть только на
машине Ивана.
...
- При смене og-image: правь `docs/og-image.html`, открой его на вьюпорте 1200×630
